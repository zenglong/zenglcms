...
<div class="article">
	...
	<div id="recommend_articles">
		...
		{if $sql->db_type == MYSQL}
			{php $sql->query("select * from " .$sql->tables_prefix . "articles where ".mytpl_unexpect_sec_sqlstr(&$this)." level=1 order by rand() limit 10");}
		{else}
			{php $sql->query("select * from " .$sql->tables_prefix . "articles where ".mytpl_unexpect_sec_sqlstr(&$this)."level=1 order by random() limit 10");}
		{/if}
		<table>
		{while $sql->parse_results()}
			{php $secId = $sql->row['sec_ID'];}
